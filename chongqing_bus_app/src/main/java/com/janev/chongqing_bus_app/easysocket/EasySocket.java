package com.janev.chongqing_bus_app.easysocket;

import android.content.Context;

import com.janev.chongqing_bus_app.easysocket.ConnectionHolder;
...
import com.janev.chongqing_bus_app.easysocket.exception.InitialExeption;
...
public class EasySocket {
     ...
     * @return
     */
    public EasySocket startHeartBeat(HeartManager.HeartbeatListener listener) {
        getDefconnection().getHeartManager().startHeartbeat(listener);
        return this;
    }
     ...
     * @return
     */
    public EasySocket startHeartBeat(String address, HeartManager.HeartbeatListener listener) {
        getConnection(address).getHeartManager().startHeartbeat(listener);
        return this;
    }
    ...
    public IConnectionManager getDefconnection() {
        if (defConnection == null) {
            throw new NotNullException("你还没有创建：" + defOptions.getSocketAddress().getIp() + ":" + defOptions.getSocketAddress().getPort()
                    + "的Socket的连接，请使用com.janev.chongqing_bus_app.easysocket.EasySocket.connect()方法创建一个默认的连接");
        }
        return defConnection;
    }
